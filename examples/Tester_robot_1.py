...
# tester robot
import os, sys
from asynch_rl.rl.resources import parallelize
...
parser.add_argument("-psm", "--peds-speeed-multiplier",dest="peds_speed_mult", type=float,default=1.1,help="pedestrians max speed multiplier")
...
def main(net_version = 100, iteration = 2, simulate = False, difficulty = 0, save_movie = False, eps_format = False, step_size = 0.4, peds_speed_mult=1.3):
    ...
    rl_env = Multiprocess_RL_Environment(env_type, model_type, net_version, rl_mode=local_vars['rl_mode'] , ray_parallelize=False, \
                                        ...
                                        difficulty= difficulty, map_output = local_vars['map_output'], \
                                          layers_width = local_vars['layers_width'], normalize_layers = local_vars['normalize_layers'] ,\
                                              rewards=local_vars['rewards'], val_frequency=local_vars['val_frequency'], step_size = step_size, peds_speed_mult=peds_speed_mult) #, \
...
if __name__ == "__main__":
    ...
    main(net_version = args.net_version, iteration = args.iteration, simulate = args.simulate, \
         difficulty = args.difficulty, save_movie=args.save_movie, eps_format=args.eps_format, step_size = args.step_size, peds_speed_mult = args.peds_speed_mult)
