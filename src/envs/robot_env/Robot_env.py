...
#%% 
#from utils.utilities_ANIMATION import AnimateClass
from src.utils.params_loader import load_pedsim_params

#%%
# Robot & Map
from src.Map import binaryOccupancyGrid , fill_surrounding
from src.Robot import DifferentialDrive
from src.RangeSensor import LiDARscanner

#%%
# pedestrians SIM
import src.extenders_py_sf.class_extender_SIM as SIM
...
def initialize_robot(visualization_angle_portion, lidar_range, lidar_n_rays, init_state, robot_collision_radius):
    ...
    # initialize peds SIM
    box_size, peds_sparsity, new_peds_params, initial_state, groups, obs, obs_llist, obstacle_avoidance_params = load_pedsim_params()
    
    sim_env = SIM.MySimulator(
        initial_state,
        groups=groups,
        obstacles=obs,
        obstacles_lolol = obs_llist,
        box_size = box_size,
        peds_sparsity = peds_sparsity,
        new_peds_params = new_peds_params,
        update_peds = True,
        obstacle_avoidance_params = obstacle_avoidance_params)
    
    #initialize map
    map_margin_coeff = 0.8 #.5
...
class RobotEnv(gym.Env):
    ...
    def step(self,action):
        ...
        """
        # updates robot coordinates in peds_sim_env (for animation purposes)
        self.robot.map.peds_sim_env.updateRobotCoord(coordinates = self.robot.getCurrentPosition())
        """
        ...
        """
        elif self.robot.checkOutOfBounds(margin = 0.05):
