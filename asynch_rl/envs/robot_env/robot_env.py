...
def initialize_robot(visualization_angle_portion, lidar_range, lidar_n_rays, init_state, robot_collision_radius,
                     peds_sparsity, diff, step_size=None):
    ...
    # initialize map
    robotMap = BinaryOccupancyGrid(map_height=2 * sim_env.box_size, map_length=2 * sim_env.box_size,
                                   peds_sim_env=sim_env)
    ...
    robot.initializeScanner(scan_range=lidar_range, angle_opening=lidar_angle_opening,
...
class RobotEnv(gym.Env):
    ...
    def __init__(self, lidar_n_rays=135, \
                 ...
                 v_linear_max=0.5, v_angular_max=1, rewards=[1, 100, 2], max_v_x_delta=.2, \
        ...
        state_max = np.concatenate((self.lidar_range * np.ones((self.lidar_n_rays,)),
                                    np.array([self.linear_max, self.angular_max, self.target_distance_max, np.pi])),
                                   axis=0)
        state_min = np.concatenate((np.zeros((self.lidar_n_rays,)), np.array([0, -self.angular_max, 0, -np.pi])),
                                   axis=0)
        ...
        self.action_space = spaces.Box(low=self.action_low, high=self.action_high,
                                       dtype=np.float64)  # , shape=(size__,))
        self.observation_space = spaces.Box(low=state_min, high=state_max,
                                            dtype=np.float64)  # , shape=(len(state_max),))
    ...
    def generate_initial_and_target_coords(self, low_bound, high_bound):
        ...
        while True:
            ...
            if np.amin(np.sqrt(np.sum((self.robot.map.obstaclesCoordinates - self.target_coordinates[:2]) ** 2,
                                      axis=1))) > min_distance:
    ...
    def reset(self, **kwargs):
        ...
        self._difficulty = self.input_difficulty if self.input_difficulty != 10 else random.randint(1,
                                                                                                    len(self.sparsity_levels) - 1)
    ...
    def render(self, mode='human', iter_params=(0, 0, 0)):
        ...
        idx_y, idx_x = \
        self.robot.map.convert_world_to_grid_no_error(np.array(robot_coords[:2])[np.newaxis, :]).tolist()[0]
        ...
        y_trg_grd, x_trg_grd = self.robot.map.convert_world_to_grid_no_error(self.target_coordinates[:2].reshape(1, 2))[
                               0, :]
        ...
        ped_dist_bar, t_target_dist_line, target_dist_bar_empty, target_dist_bar = \
            render_image(self.robot.map.map_length, self.robot.map.map_height, occupancy_map, idx_x, idx_y, robot_angle,
                         scan_pts_grid, closest_obstacle, \
        ...
        self.data_render.append(
            [occupancy_map, idx_x, idx_y, robot_angle, scan_pts_grid, closest_obstacle, rel_width, x_trg_grd, y_trg_grd,
             iter_params])
...
def render_image(map_width, map_height, occupancy_map, idx_x, idx_y, robot_angle, scan_pts_grid, closest_obstacle, \
    ...
    rect = patches.Rectangle((idx_x - robot_width / 2, idx_y - robot_height / 2), robot_width, robot_height, 0,
                             linewidth=0.3, edgecolor='k', facecolor='b')

    transf_rect = mpl.transforms.Affine2D().rotate_deg_around(idx_x, idx_y,
                                                              np.degrees(robot_angle)) + img.axes.transData
    rect.set_transform(transf_rect)
    # print(robot_angle)
    rect_direction = patches.Arrow(idx_x, idx_y, arrow_len * np.cos(robot_angle), arrow_len * np.sin(robot_angle),
                                   width=10)
    ...
    ped_dist_line_empty = patches.Rectangle((190, 20), dist_bar_max_width, dist_bar_height, 0, edgecolor='k',
                                            facecolor='w')
    ped_dist_line = patches.Rectangle((190, 20), rel_width * dist_bar_max_width, dist_bar_height, 0, edgecolor='k',
                                      facecolor='r')
    ped_dist_bar_empty = img.axes.add_patch(ped_dist_line_empty)
    ped_dist_bar = img.axes.add_patch(ped_dist_line)

    t_target_dist_line = img.axes.text(190, 45, 'target reach = ' + str(round(target_distance, 1)), fontsize=10,
                                       color='k')

    target_dist_line_empty = patches.Rectangle((190, 50), dist_bar_max_width, dist_bar_height, 0, edgecolor='k',
                                               facecolor='w')
    target_dist_line = patches.Rectangle((190, 50), target_rel_width * dist_bar_max_width, dist_bar_height, 0,
