...
class ConvModel(nnBase):
    ...
    def get_net_input_shape(self):
        return (1, self.channels_in, self.n_frames, self.N_in[0] ), (1, self.N_in[1])
    ...
    def update_NN_structure(self):
        ...
        for i in range(len(self.channels)):
            if i == 0:
                ...
                layer = nn.Conv2d(in_channels= self.channels[i-1], out_channels=self.channels[i], \
        ...
        conv_output_len = x_test.flatten().shape[0]
                
        if self.partial_outputs:
            self.fc_po_1 = nn.Linear(conv_output_len, 20)
            x_test_po = self.fc_po_1(x_test.flatten())
            self.po_norm_layer = nn.LayerNorm(x_test_po.shape[-1])
            self.fc_po_2 = nn.Linear(20, self.n_partial_outputs)        
                
        #N_linear_in = round(self.N_in[0]/(5*3**(len(self.channels)-1))*self.channels[-1] + self.N_in[1])
        N_linear_in = conv_output_len + self.N_in[1] 

        # fully connected layers - strategy
    ...
    def conv_forward(self,x):
        ...
        if self.partial_outputs:
            ...
            po = self.po_norm_layer(F.relu(self.fc_po_1(x1.flatten(1))))
            po = torch.sigmoid(self.fc_po_2(po))
            
            return torch.cat((x1.flatten(1), x[1].flatten(1)),dim = 1) , po
        else:
